		...
		<div class="row" style=''>
			<div class=" col-lg-8 col-md-8 col-sm-12 col-xs-12 " id="makePane">
				<div class='thumbnail thumbnail-tune school-paper solid-light-two' style="">
					...
					<form action='{{ route('save.note',Auth::user()->id) }}' method='get'>
						<div class='form-group clearfix'>
							<br>
							<label for='' class="dark-knight">Title</label>
							<input type='name' class='form-control solid-rank'  id ='piece-title' placeholder='title' name='title'>
							<br>
							<label for='' class="dark-knight" >Note</label>
							<textarea rows='13' class='form-control solid-rank clearfix' id ='piece-body' name='note' placeholder='Express yourself'></textarea>
							<hr class='normal-merge-div'>
								<button class='btn btn-danger solid-rank' type='button' data-toggle='modal' data-target='#skeleton-modal'><span class='glyphicon glyphicon-sunglasses' data-toggle='tooltip' title='Your can view your piece in skeleton form over here!' data-placement ='right'></span></button>
							...
							<button class='btn btn-success pull-right solid-rank' type='submit'><span class='glyphicon glyphicon-save'></span> Save</button>
								{{-- <button type='button' class='btn btn-default' id='searchButton'><span class='glyphicon glyphicon-search'></span> call</button> --}}
						</div>
					</form>
					<p id='showText' > </p>
			...
			<div class='col-lg-4 col-md-4 col-sm-12 col-xs-12'>
				...
				<div class='thumbnail  thumbnail-tune' >
					...
					id='Foreshadowing-sticker' class='label label-danger solid-rank eng-tips dark-knight' style='background-color:cyan '  data-title='Foreshadowing' data-toggle='popover' data-placement='top' data-html='true' data-content="
					...
					@if(Session::has('DeviceBag'))
						...
						@foreach (Session::get('DeviceBag')->devices as $device)
							...
							@php
								switch ($device['name']) {
									...
									case 'Personification':
										echo "<h3 class='label label-warning solid-rank' style='background-color: red; color: black;' >".$device['name']."</h3><br><br>";
										break;
									case 'Epigraph':
							...
							@foreach ( explode(':'.$device['name'].'-mark'.':', $device['text'] ) as $deviceText)
								@php
									$number = $number +1;
									echo "<p> ".$number.". ".$deviceText." <button data-mother-ID='".lookForMother($device['name'])."' data-ID ='".$number."' class='btn btn-default btn-xs'><span class='glyphicon glyphicon-minus'></button></p>";
									echo lookForMother($device['name']) ;
								@endphp
							@endforeach
						@endforeach
					@else
